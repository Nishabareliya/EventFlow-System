...
export default function Booking() {
  ...
  const hanldeSumit = async (e) => {
    ...
    console.log(user);
    try {
      const response = await fetch(`http://localhost:5000/event/api/users`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const eventTypesResponse = await fetch('http://localhost:5000/api/event');
        const eventTypesData = await eventTypesResponse.json();
        setEventTypes(eventTypesData);
        console.log("event_Type",eventTypes);

        // Fetch venues from the backend API
        const venuesResponse = await fetch('http://localhost:5000/api/event');
  ...
  return (
    ...
    <Navbar></Navbar>
      <div className='photo'>
        ...
        <h1 style={{marginTop: "-70px"}}>Plan Your Ideal Book Event! </h1>
